...
| Organic Dominance §19 **Reputation & Crisis Command** (13 early-warning signals; Crisis Severity Score from 10 factors; 4-level crisis workflows Monitor→Executive) | Streamed spec 2026-07-20 | ✅ partial | reputation scoring in `reputation.ts` + risk in `market-listening.ts`; dedicated Crisis Severity Score + 4-level workflow 📦 (next-round `crisis-command`) |
...
| Organic Dominance §21 **Customer Voice Intelligence** (merge 12 proprietary+public inputs → 13 outputs; Product Backlog Bridge: insight → product requirement) | Streamed spec 2026-07-20 | 📦 | next-round `customer-voice` engine (deterministic pain-clustering + backlog-bridge core) |
| Organic Dominance §22 **Influencer & Creator Intelligence** (11 discovery signals; campaign management incl. fraud detection; micro-influencer/local-first priority) | Streamed spec 2026-07-20 | 📦 | next-round `creator-intel` engine (deterministic scoring; discovery/ingestion connector-gated) |
...
| Subscription + ACU hybrid tiers: Starter 500 ACUs / Growth 5,000 / Business 25,000 / Enterprise negotiated; unused ACUs expire | Part 12 L7842–7894; Part 15 L19069–19093 | ✅ partial | mechanism in `src/backend/acu.ts` (`PLANS`); **specific allotments/prices are indicative placeholders — owner finalises at launch (owner directive 2026-07-19)** |
