...
export const ENV_CATALOGUE: EnvVar[] = [
  ...
  { name: "HUNTER_API_KEY", group: "Data", secret: true, unlocks: "The email finder AND a real mailbox verifier — the one thing the crawl and the register cannot do. Runs LAST in the contact waterfall, so a paid credit is only spent on what the free sources missed.", where: "hunter.io — free tier to test, then bulk credits with no subscription." },
  ...
  { name: "MW_REPLY_HOST", group: "Site", secret: false, unlocks: "Replies. UNTIL THIS IS SET NO REPLY ADDRESS IS ISSUED and campaigns reply to the sender's own From address — because an address on a host with no MX bounces the reply back to the prospect. It used to default to reply.marketwaros.com, which has no MX.", where: "A subdomain you control whose MX points at the inbound intake. Publish the MX first, then set this." },
  { name: "MW_MX_HOST", group: "Site", secret: false, unlocks: "The reply MX row in Sending Domains. With this unset the row is not shown at all, rather than naming a host that cannot receive mail and being refused by the customer's registrar.", where: "The hostname of a server that actually accepts SMTP on port 25 — not a web host." },
  ...
  { name: "NEXT_PUBLIC_SOCIAL_LINKEDIN", group: "Legal", secret: false, unlocks: "Binds this company to its LinkedIn page in the structured data (sameAs), so search engines and AI assistants treat them as ONE entity instead of guessing. This platform's own AI-citation module tells customers they must have this.", where: "The full https:// URL of your company page. Leave unset rather than guessing — a sameAs pointing at somebody else's profile is a machine-readable lie." },
  { name: "NEXT_PUBLIC_SOCIAL_X", group: "Legal", secret: false, unlocks: "Binds this company to its X/Twitter profile in the structured data (sameAs), so search engines and AI assistants treat the two as ONE entity rather than inferring it from prose. Also fills the social links the site audit scores every page on.", where: "The full https:// profile URL. Leave it unset rather than guessing — a sameAs pointing somewhere that is not yours is a machine-readable lie." },
  { name: "NEXT_PUBLIC_SOCIAL_FACEBOOK", group: "Legal", secret: false, unlocks: "Binds this company to its Facebook profile in the structured data (sameAs), so search engines and AI assistants treat the two as ONE entity rather than inferring it from prose. Also fills the social links the site audit scores every page on.", where: "The full https:// profile URL. Leave it unset rather than guessing — a sameAs pointing somewhere that is not yours is a machine-readable lie." },
  { name: "NEXT_PUBLIC_SOCIAL_INSTAGRAM", group: "Legal", secret: false, unlocks: "Binds this company to its Instagram profile in the structured data (sameAs), so search engines and AI assistants treat the two as ONE entity rather than inferring it from prose. Also fills the social links the site audit scores every page on.", where: "The full https:// profile URL. Leave it unset rather than guessing — a sameAs pointing somewhere that is not yours is a machine-readable lie." },
  { name: "NEXT_PUBLIC_SOCIAL_YOUTUBE", group: "Legal", secret: false, unlocks: "Binds this company to its YouTube profile in the structured data (sameAs), so search engines and AI assistants treat the two as ONE entity rather than inferring it from prose. Also fills the social links the site audit scores every page on.", where: "The full https:// profile URL. Leave it unset rather than guessing — a sameAs pointing somewhere that is not yours is a machine-readable lie." },
  { name: "NEXT_PUBLIC_COMPANIES_HOUSE_URL", group: "Legal", secret: false, unlocks: "Binds the site to the official company register entry — the strongest entity signal there is, because it is a third party nobody can self-certify.", where: "The full https:// URL of your Companies House record." },
  { name: "REQUIRE_LIVE", group: "AI", secret: false, unlocks: "Refuses to serve deterministic demo AI output to real users. With it set, an agent whose provider is momentarily unreachable shows an honest 'activating' message instead of canned text that reads like a real answer. Declared for Firebase App Hosting and absent from this registry until a test compared the two hosting configs — so anyone configuring the OTHER deployment could not have known it existed.", where: "Set to 1 on any deployment serving real customers." },
  { name: "INDEXNOW_KEY", group: "Site", secret: false, unlocks: "Announcing new and changed pages to Bing, Yandex, Seznam and Naver the moment they publish, instead of waiting to be crawled. Google is NOT a participant. Public by design — the protocol requires the key to be served at /indexnow-key.txt as proof you own the site.", where: "Invent one: any 8-128 letters, digits or hyphens. It only has to stay the same." },
  { name: "MW_SEED_MAILBOXES", group: "Email", secret: true, unlocks: "MEASURING WHERE OUR MAIL ACTUALLY LANDS. Without it the platform can describe the SHAPE of a message — authentication, stream, headers, text part — and cannot tell you which folder a real receiver put it in. A JSON array of mailboxes you own, one per receiver you care about: [{\"address\":\"you@gmail.com\",\"host\":\"imap.gmail.com\",\"port\":993,\"user\":\"you@gmail.com\",\"pass\":\"<app password>\"}]. Gmail matters most, because its Promotions tab is the question customers ask about.", where: "Create one free mailbox per receiver (Gmail, Outlook, Yahoo) and generate an app password for each — never the account password." },
  { name: "MW_BOUNCE_IMAP_HOST", group: "Email", secret: false, unlocks: "COLLECTING DELIVERY FAILURES AUTOMATICALLY. Without it nothing reads the bounce mailbox, so the only way to learn why a message failed is to open that mailbox by hand — which is not a platform. Every failure notice already arrives there; this is the reader.", where: "The IMAP server of the mailbox your mail is sent from (e.g. imap.your-mail-host.com)." },
  { name: "MW_BOUNCE_IMAP_PORT", group: "Email", secret: false, unlocks: "The IMAP port. Defaults to 993 (implicit TLS), which is what almost every host uses.", where: "Your mail host's IMAP settings." },
  { name: "MW_BOUNCE_IMAP_USER", group: "Email", secret: false, unlocks: "The mailbox to read. Defaults to SMTP_USER, because the envelope sender is that account and the failures already land in it.", where: "Only needed if bounces go to a different mailbox from the one that sends." },
  { name: "MW_BOUNCE_IMAP_PASS", group: "Email", secret: true, unlocks: "The password for that mailbox. Defaults to SMTP_PASS for the same reason.", where: "Only needed if bounces go to a different mailbox from the one that sends." },
  { name: "MW_BOUNCE_IMAP_MAILBOX", group: "Email", secret: false, unlocks: "Which folder to read. Defaults to INBOX.", where: "Only if your host files delivery notices somewhere else." },
