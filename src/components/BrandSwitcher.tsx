...
// Adding a brand captures the essentials the engines need; no third-party key
// is involved.
...
export default function BrandSwitcher() {
  const { brands, activeBrand, setActive, addBrand } = useActiveBrand();
  const [open, setOpen] = useState(false);
  const [adding, setAdding] = useState(false);
  const [form, setForm] = useState({ name: "", industry: "", product: "", audience: "", location: "", offer: "", website: "", goal: "" });
  ...
  function submit() {
    ...
    addBrand(form);
    setForm({ name: "", industry: "", product: "", audience: "", location: "", offer: "", website: "", goal: "" });
    setAdding(false);
    setOpen(false);
  }
          ...
          {!adding ? (
            <>
              <div className="max-h-72 overflow-y-auto">
                {brands.map((b) => (
                  <button
                    ...
                    className={`flex w-full items-center gap-2 rounded-md px-2.5 py-2 text-left text-sm transition ${b.id === active?.id ? "bg-emerald-500/10 text-white" : "text-slate-300 hover:bg-ink-850"}`}
                  >
              ...
              <div className="mt-1 border-t border-ink-700/60 pt-1">
                <button type="button" onClick={() => setAdding(true)} className="flex w-full items-center gap-2 rounded-md px-2.5 py-2 text-left text-xs font-semibold text-emerald-400 transition hover:bg-ink-850">
            ...
            <div className="p-1.5">
              <div className="mb-2 flex items-center justify-between">
                ...
                <button type="button" onClick={() => setAdding(false)} className="text-slate-500 hover:text-white"><X className="h-3.5 w-3.5" /></button>
              </div>
