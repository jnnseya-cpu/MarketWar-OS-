...
## §109 — We failed the checks we sell (2026-09-06)

### The method
Asked to look over the public and marketing surface, the sharpest available test
was not an opinion: this platform ships a 30-check website audit. So the audit
was pointed at our own pages — a production build on `next start`, crawled by
`crawlSite` itself with `allowLoopback` under `NODE_ENV=test`.

Fourteen public pages, before:

| | |
|---|---|
| Landing page | 78/100 B |
| Best page | 81/100 B |
| Worst page (`/partner`) | 70/100 C |
| Failing on almost every page | **Open Graph**, Twitter card, Canonical tag |

### 1. Every share of this site rendered as a bare grey link
There was no `openGraph` and no `twitter` block anywhere in the app. A link to
marketwaros.com posted in LinkedIn, Facebook, WhatsApp, Slack, iMessage or X
showed a URL and nothing else.

**`public/brand/social/og-card.png` had existed the whole time** — 1200×630,
generated by `npm run social` — and nothing in the codebase referenced it. The
card was made and never connected. The same shape as the missing llms.txt: we
were failing the check we sell, on the platform that sells social publishing.

### 2. Two canonical hosts in one app
`metadataBase` was the APEX while `siteOrigin()` — which robots.txt, sitemap.xml
and every JSON-LD block use — is `www`. Relative Open Graph and canonical URLs
therefore resolved to a host the sitemap does not advertise. It is the same
apex/www split already suspected behind the Stripe webhook, and it is now fixed
at the single source rather than in a second place that can drift.

### 3. A Next.js trap: a partial `openGraph` silently drops the image
Metadata merges field by field across segments, but `openGraph` is **replaced
wholesale**. Six places set `{ title, description, type }` and lost the root
layout's `images` with no error and source that looks correct: `/how-it-works`,
`/audit`, `/choose-plan`, `/contact`, `/partner`, every article and every answer
page. Measured on a running build: `og:title=1, og:image=0` on all four checked.

`openGraphFor()` in `shared/site.ts` now returns the complete block; the image
cannot be omitted by forgetting it. A test walks `src/app` and fails on any
`openGraph:` outside the root layout that does not go through it.

### 4. Our own meta-description rule, broken on our own pages
`audit-copy.ts` tells customers to "write 50–165 characters" and the crawler
scores them on it. Eight of our own pages were outside it — the landing page at
**205**, `/features` 214, `/audit` 210, `/share2earn` 208, `/growth` 205,
`/how-it-works` 198, `/verify-human` 203, and `/login` too short at 43. Google
truncates past ~160, so on every one of them the last third was never read; on
the landing page the cut-off clause was "tells you exactly what to do next".

All eight rewritten inside the bounds. The test reads `min`/`max` **out of the
crawler**, so changing the rule moves the test rather than leaving it asserting a
number nobody maintains — and it found the two pages that manual measurement had
missed.

### 5. Sitemap: the last step of the funnel missing, a gated tool advertised
`/get-started` — its own metadata, the marketing shell, the four steps — was
never added, so search engines were not offered the page that turns interest into
an account. Meanwhile `/partner` **was** listed: a `"use client"` tool whose own
header says the access code "is the only credential", that nothing in the app
links to, scoring 70/100 with 914 words of shell where the landing page has
13,489. Listing it spent crawl budget on an empty dashboard. `/share2earn` is the
public pitch and keeps that traffic.

### After
Open Graph, Twitter card and Canonical are gone from every page's findings.
Landing page 78 → **83**, `/about` 81 → **85**, `/developers` 81 → **85**,
`/contact` 82 → **83**, and the pages that had been failing Open Graph outright
gained 4–6 points each. Nine mutations killed.

### Deliberately NOT changed — the owner's calls, not mine
Three findings remain on every page and none of them is something code should
decide:

- **Phone number** and **Local address** — a UK company with neither on its
  public pages. Real, and it costs trust and local search, but I do not have the
  number or the address and inventing them is the one thing this platform exists
  to argue against.
- **Social profiles** — no links in the footer. The check's own instruction is
  "the active ones only", and I do not know which accounts exist.
- **Render-blocking scripts** — Next's own bundles, on every page. Worth a look
