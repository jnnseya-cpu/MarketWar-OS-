...
const PROVISIONING: Record<IntegrationProvider, ProvisioningMeta> = {
  ...
  facebook_pages: { provisioning: "user_connect", billing: "included", platformEnvKey: "FB_APP_ID", userAction: "Connect with Facebook (or paste a Page token) in the Publish Center — posts go through MarketWar's own Meta connector, no vendor.", reason: "Native connector via the Graph API — zero per-post vendor cost. Zernio is the failover; the manual path is always available." },
  instagram_business: { provisioning: "user_connect", billing: "included", platformEnvKey: "FB_APP_ID", userAction: "Connect once with Facebook (Instagram Business is linked to your Page) — publishes natively when a creative image is attached.", reason: "Native connector via the Graph API (one Meta connect covers Facebook + Instagram). Zernio is the failover." },
